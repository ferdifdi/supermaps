"""Isochrone jalan kaki 800m + rute + jaringan jalan untuk semua stasiun KRL (Jabodetabek).

Install dulu (sekali saja): pip install -r requirements.txt   (isinya cuma pyshp)

Jalankan: python isochrone_krl.py
...
sebagai data statis, gak usah panggil Overpass lagi tiap generate use case):
  1. isochrone_krl_stations[.shp|.geojson]        (Point)     - titik tiap stasiun KRL
  2. isochrone_krl_routes[.shp|.geojson]          (PolyLine)  - jalur rel KRL (OSM route=train)
  3. isochrone_krl_isochrone_800m[.shp|.geojson]  (Polygon)   - jangkauan jalan kaki 800m/stasiun
  4. isochrone_krl_roads_800m[.shp|.geojson]      (PolyLine)  - ruas jalan yang ada di dalam
...
Progress disimpan checkpoint di isochrone_krl_work.json (working file internal, bukan
...
`python isochrone_krl.py` - otomatis skip yang sudah selesai, cuma retry yang gagal, lalu
tulis ulang ke-4 shapefile dari data terbaru. Daftar yang masih gagal dicatat di
isochrone_krl_800m_failed.txt.
"""
...
MODE = "KRL"
ROUTE_TAG = "train"  # OSM route=* value buat relasi jalur KRL
CUTOFF_M = 800
RADIUS_QUERY = 1000  # radius fetch OSM - harus lebih besar dari CUTOFF_M biar graf gak kepotong pas di tepi
OUT_DIR = Path(__file__).resolve().parent
WORK_FILE = OUT_DIR / "isochrone_krl_work.json"
FAILED_FILE = OUT_DIR / "isochrone_krl_800m_failed.txt"
STATIONS_SHP = OUT_DIR / "isochrone_krl_stations"
ROUTES_SHP = OUT_DIR / "isochrone_krl_routes"
ISOCHRONE_SHP = OUT_DIR / "isochrone_krl_isochrone_800m"
ROADS_SHP = OUT_DIR / "isochrone_krl_roads_800m"
STATIONS_GEOJSON = OUT_DIR / "isochrone_krl_stations.geojson"
ROUTES_GEOJSON = OUT_DIR / "isochrone_krl_routes.geojson"
ISOCHRONE_GEOJSON = OUT_DIR / "isochrone_krl_isochrone_800m.geojson"
ROADS_GEOJSON = OUT_DIR / "isochrone_krl_roads_800m.geojson"
CHECKPOINT_EVERY = 20
...
async def fetch_route_lines(route_tag: str) -> list[dict]:
    ...
    route=train di OSM dipakai PT KAI buat SEMUA kereta, bukan cuma KRL - kereta jarak
    jauh (Argo Bromo Anggrek Gambir-Surabaya, Gajayana Gambir-Malang, dst) juga ke-tag
    sama dan relasinya lewat/nyentuh bbox Jabodetabek. Yang beneran KRL Commuter Line
    operatornya konsisten "KAI Commuter" (atau nama lama "PT KCJ") - kereta jarak jauh
    operatornya cuma "KAI"/"Kereta Api Indonesia" tanpa "Commuter". Filter ini di query
    Overpass-nya sendiri (bukan post-filter) biar gak narik ribuan titik geometri kereta
    lintas Jawa yang gak kepake."""
    operator_filter = '["operator"~"commuter|kcj",i]' if route_tag == "train" else ""
    q = f"""
    [out:json][timeout:90];
    rel["route"="{route_tag}"]{operator_filter}({osm._bbox()});
    out body;
    >;
    out geom qt;
    """
