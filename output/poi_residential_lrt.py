"""POI bangunan residential (building=residential/apartments/house) buat stasiun LRT -
MAPID gak punya kategori perumahan/bangunan, jadi ini tetap OSM, didownload sekali jadi
static dalam 800m dari tiap stasiun LRT, biar backend gak perlu manggil osm.pois() live
...
Jalankan: python poi_residential_lrt.py
Output: poi_residential_lrt_800m.geojson (folder yang sama dengan script ini)
...
poi_residential_lrt_work.json - kalau ke-stop/gagal di tengah jalan, jalankan lagi
`python poi_residential_lrt.py`, otomatis lanjut dari stasiun yang belum selesai.
"""
...
WORK_FILE = OUT_DIR / "poi_residential_lrt_work.json"
OUTPUT_FILE = OUT_DIR / "poi_residential_lrt_800m.geojson"
FAILED_FILE = OUT_DIR / "poi_residential_lrt_800m_failed.txt"
...
async def target_stations() -> list[dict]:
    """LRT stations (OSM) - TJ (this project's poi_residential_tj.py) is the only mode
