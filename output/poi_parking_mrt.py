"""POI parkir (mobil + motor) buat stasiun MRT - MAPID gak punya kategori parkir, jadi ini
tetap OSM, didownload sekali jadi static dalam 800m dari tiap stasiun MRT, biar
...
Jalankan: python poi_parking_mrt.py
Output: poi_parking_mrt_800m.geojson (folder yang sama dengan script ini)
...
poi_parking_mrt_work.json - kalau ke-stop di tengah jalan (proses dimatikan paksa dkk),
jalankan lagi `python poi_parking_mrt.py`, otomatis lanjut dari stasiun yang belum selesai.
...
permanen mulai run berikutnya - lihat poi_parking_mrt_giveup.json - biar stasiun yang emang
...
WORK_FILE = OUT_DIR / "poi_parking_mrt_work.json"
OUTPUT_FILE = OUT_DIR / "poi_parking_mrt_800m.geojson"
FAILED_FILE = OUT_DIR / "poi_parking_mrt_800m_failed.txt"
GIVEUP_FILE = OUT_DIR / "poi_parking_mrt_giveup.json"
CHECKPOINT_EVERY = 30
...
async def target_stations() -> list[dict]:
    """MRT stations (OSM) - TJ (this project's poi_parking_tj.py) is the only mode
