...
berteduh (shelter) - dalam 800m dari tiap stasiun KRL (bukan 800m kayak MRT/KRL/LRT - lihat
...
poi_green_krl_work.json - kalau ke-stop di tengah jalan (proses dimatikan paksa dkk),
jalankan lagi `python poi_green_krl.py`, otomatis lanjut dari stasiun yang belum selesai.
...
permanen mulai run berikutnya - lihat poi_green_krl_giveup.json - biar stasiun yang emang
...
GIVEUP_FILE = OUT_DIR / "poi_green_krl_giveup.json"
CHECKPOINT_EVERY = 30
...
async def target_stations() -> list[dict]:
    """KRL stations (OSM) - TJ (this project's poi_green_tj.py) is the only mode
