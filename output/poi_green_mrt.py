"""POI ruang hijau buat stasiun MRT: pohon individual, taman/kebun/hutan kota, tempat
berteduh (shelter) - dalam 800m dari tiap stasiun MRT (bukan 800m kayak MRT/KRL/LRT - lihat
...
Jalankan: python poi_green_mrt.py
Output: poi_green_mrt_800m.geojson (folder yang sama dengan script ini)
...
poi_green_mrt_work.json - kalau ke-stop di tengah jalan (proses dimatikan paksa dkk),
jalankan lagi `python poi_green_mrt.py`, otomatis lanjut dari stasiun yang belum selesai.
...
permanen mulai run berikutnya - lihat poi_green_mrt_giveup.json - biar stasiun yang emang
...
WORK_FILE = OUT_DIR / "poi_green_mrt_work.json"
OUTPUT_FILE = OUT_DIR / "poi_green_mrt_800m.geojson"
FAILED_FILE = OUT_DIR / "poi_green_mrt_800m_failed.txt"
GIVEUP_FILE = OUT_DIR / "poi_green_mrt_giveup.json"
CHECKPOINT_EVERY = 30
...
async def target_stations() -> list[dict]:
    """MRT stations (OSM) - TJ (this project's poi_green_tj.py) is the only mode
