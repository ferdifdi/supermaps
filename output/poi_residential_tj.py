...
static dalam 800m dari tiap halte TJ, biar backend gak perlu manggil osm.pois() live
tiap kali `population_density`/`residential_diversity` (K-UC1) atau residential_mix
(M-UC1 walk_score) dihitung.

Install dulu (sekali saja): pip install -r requirements.txt

Jalankan: python poi_residential_tj.py
Output: poi_residential_tj_800m.geojson (folder yang sama dengan script ini)
...
halte) - job ini lebih berat dari poi_green_*.py/poi_parking_*.py, DAN TJ punya ~8091
halte (di-thin dulu grid 500m, tapi tetap job PALING BESAR dari semua script poi_*.py
gabungan). Wajar makan waktu paling lama dari semuanya.

Butuh koneksi internet (Overpass API). Progress disimpan checkpoint di
poi_residential_tj_work.json - kalau ke-stop/gagal di tengah jalan, jalankan lagi
`python poi_residential_tj.py`, otomatis lanjut dari stasiun yang belum selesai.
"""
...
OUTPUT_FILE = OUT_DIR / "poi_residential_tj_800m.geojson"
FAILED_FILE = OUT_DIR / "poi_residential_tj_800m_failed.txt"
