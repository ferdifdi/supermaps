"""Isochrone jalan kaki 800m + rute + jaringan jalan untuk semua stasiun LRT (Jabodetabek).

Install dulu (sekali saja): pip install -r requirements.txt   (isinya cuma pyshp)

Jalankan: python isochrone_lrt.py
...
sebagai data statis, gak usah panggil Overpass lagi tiap generate use case):
  1. isochrone_lrt_stations[.shp|.geojson]        (Point)     - titik tiap stasiun LRT
  2. isochrone_lrt_routes[.shp|.geojson]          (PolyLine)  - jalur rel LRT (OSM route=light_rail)
  3. isochrone_lrt_isochrone_800m[.shp|.geojson]  (Polygon)   - jangkauan jalan kaki 800m/stasiun
  4. isochrone_lrt_roads_800m[.shp|.geojson]      (PolyLine)  - ruas jalan yang ada di dalam
...
Progress disimpan checkpoint di isochrone_lrt_work.json (working file internal, bukan
...
`python isochrone_lrt.py` - otomatis skip yang sudah selesai, cuma retry yang gagal, lalu
tulis ulang ke-4 shapefile dari data terbaru. Daftar yang masih gagal dicatat di
isochrone_lrt_800m_failed.txt.
"""
...
MODE = "LRT"
ROUTE_TAG = "light_rail"  # OSM route=* value buat relasi jalur LRT
CUTOFF_M = 800
RADIUS_QUERY = 1000  # radius fetch OSM - harus lebih besar dari CUTOFF_M biar graf gak kepotong pas di tepi
OUT_DIR = Path(__file__).resolve().parent
WORK_FILE = OUT_DIR / "isochrone_lrt_work.json"
FAILED_FILE = OUT_DIR / "isochrone_lrt_800m_failed.txt"
STATIONS_SHP = OUT_DIR / "isochrone_lrt_stations"
ROUTES_SHP = OUT_DIR / "isochrone_lrt_routes"
ISOCHRONE_SHP = OUT_DIR / "isochrone_lrt_isochrone_800m"
ROADS_SHP = OUT_DIR / "isochrone_lrt_roads_800m"
STATIONS_GEOJSON = OUT_DIR / "isochrone_lrt_stations.geojson"
ROUTES_GEOJSON = OUT_DIR / "isochrone_lrt_routes.geojson"
ISOCHRONE_GEOJSON = OUT_DIR / "isochrone_lrt_isochrone_800m.geojson"
ROADS_GEOJSON = OUT_DIR / "isochrone_lrt_roads_800m.geojson"
CHECKPOINT_EVERY = 20
