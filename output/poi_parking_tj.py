...
tetap OSM, didownload sekali jadi static dalam 400m dari tiap halte TJ (bukan 800m kayak
MRT/KRL/LRT - lihat static_transit.MODE_RADIUS_M, TJ halte-nya jauh lebih rapat), biar
backend gak perlu manggil osm.pois() live tiap kali `car_parking`/`motorcycle_parking`
(K-UC1) dihitung.

PERINGATAN: TJ punya ~2876 halte (3 kategori GTFS: Angkutan Umum Integrasi, BRT, Transjabodetabek - Mikrotrans dkk di luar scope) mentah dari GTFS - di-thin dulu (grid 250m) sebelum
...
Output: poi_parking_tj_400m.geojson (folder yang sama dengan script ini)
...
poi_parking_tj_work.json - kalau ke-stop di tengah jalan (proses dimatikan paksa dkk),
jalankan lagi `python poi_parking_tj.py`, otomatis lanjut dari stasiun yang belum selesai.
Halte yang gagal (timeout/error Overpass, bukan dimatikan paksa) di-retry OTOMATIS di
...
jeda REQUEST_DELAY_S biar gak nembak Overpass beruntun sampai kena rate-limit/502. Halte
yang TETAP gagal di >= GIVEUP_THRESHOLD run terpisah (bukan cuma di dalam satu run) di-skip
permanen mulai run berikutnya - lihat poi_parking_tj_giveup.json - biar halte yang emang
...
OUTPUT_FILE = OUT_DIR / "poi_parking_tj_400m.geojson"
FAILED_FILE = OUT_DIR / "poi_parking_tj_400m_failed.txt"
GIVEUP_FILE = OUT_DIR / "poi_parking_tj_giveup.json"
CHECKPOINT_EVERY = 30
...
# Failed halte (Overpass timeout/error) get retried automatically - once right after each
...
# caps it so a halte that fails for a non-transient reason (e.g. consistently malformed
...
# ~2876-halte run). A flat per-request delay is cheap insurance against that.

THIN_CELL_M = 250  # < RADIUS_M so a dropped station's 400m circle is still ~fully
# covered by the kept representative's circle (worst-case corner-to-corner gap in a
# 250m grid cell is ~354m, comfortably under 400m).
...
async def _process(s: dict, points_by_key: dict, done_ids: set) -> bool:
    """Fetches one halte, folding results into points_by_key/done_ids on success.
...
async def main():
    ...
    # Bump each still-failed halte's cross-run counter - this is what eventually excludes
    # a persistently-broken halte from `remaining` in a future run (see GIVEUP_THRESHOLD).
    # A halte that succeeds on some other run (even after failing here) never accumulates
