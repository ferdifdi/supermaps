...
berteduh (shelter) - dalam 400m dari tiap halte TJ (bukan 800m kayak MRT/KRL/LRT - lihat
static_transit.MODE_RADIUS_M, TJ halte-nya jauh lebih rapat), biar backend gak perlu
manggil osm.trees()/osm.pois() live tiap kali M-UC1 "Kenyamanan" tab dibuka.
...
Output: poi_green_tj_400m.geojson (folder yang sama dengan script ini)
...
poi_green_tj_work.json - kalau ke-stop di tengah jalan (proses dimatikan paksa dkk),
jalankan lagi `python poi_green_tj.py`, otomatis lanjut dari stasiun yang belum selesai.
...
permanen mulai run berikutnya - lihat poi_green_tj_giveup.json - biar halte yang emang
...
OUTPUT_FILE = OUT_DIR / "poi_green_tj_400m.geojson"
FAILED_FILE = OUT_DIR / "poi_green_tj_400m_failed.txt"
GIVEUP_FILE = OUT_DIR / "poi_green_tj_giveup.json"
CHECKPOINT_EVERY = 30
