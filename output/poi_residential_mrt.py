"""POI bangunan residential (building=residential/apartments/house) buat stasiun MRT -
MAPID gak punya kategori perumahan/bangunan, jadi ini tetap OSM, didownload sekali jadi
static dalam 800m dari tiap stasiun MRT, biar backend gak perlu manggil osm.pois() live
...
Jalankan: python poi_residential_mrt.py
Output: poi_residential_mrt_800m.geojson (folder yang sama dengan script ini)
...
stasiun) - job ini lebih berat dari poi_green_*.py/poi_parking_*.py, wajar makan waktu
lebih lama.

Butuh koneksi internet (Overpass API). Progress disimpan checkpoint di
poi_residential_mrt_work.json - kalau ke-stop/gagal di tengah jalan, jalankan lagi
`python poi_residential_mrt.py`, otomatis lanjut dari stasiun yang belum selesai.
"""
...
WORK_FILE = OUT_DIR / "poi_residential_mrt_work.json"
OUTPUT_FILE = OUT_DIR / "poi_residential_mrt_800m.geojson"
FAILED_FILE = OUT_DIR / "poi_residential_mrt_800m_failed.txt"
...
async def target_stations() -> list[dict]:
    """MRT stations (OSM) - TJ (this project's poi_residential_tj.py) is the only mode
