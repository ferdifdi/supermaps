"""POI parkir (mobil + motor) buat stasiun KRL - MAPID gak punya kategori parkir, jadi ini
tetap OSM, didownload sekali jadi static dalam 800m dari tiap stasiun KRL, biar
...
Jalankan: python poi_parking_krl.py
Output: poi_parking_krl_800m.geojson (folder yang sama dengan script ini)
...
poi_parking_krl_work.json - kalau ke-stop di tengah jalan (proses dimatikan paksa dkk),
jalankan lagi `python poi_parking_krl.py`, otomatis lanjut dari stasiun yang belum selesai.
...
permanen mulai run berikutnya - lihat poi_parking_krl_giveup.json - biar stasiun yang emang
...
WORK_FILE = OUT_DIR / "poi_parking_krl_work.json"
OUTPUT_FILE = OUT_DIR / "poi_parking_krl_800m.geojson"
FAILED_FILE = OUT_DIR / "poi_parking_krl_800m_failed.txt"
GIVEUP_FILE = OUT_DIR / "poi_parking_krl_giveup.json"
CHECKPOINT_EVERY = 30
...
async def target_stations() -> list[dict]:
    """KRL stations (OSM) - TJ (this project's poi_parking_tj.py) is the only mode
