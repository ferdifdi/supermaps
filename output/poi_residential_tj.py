"""POI bangunan residential (building=residential/apartments/house) buat halte TJ -
MAPID gak punya kategori perumahan/bangunan, jadi ini tetap OSM, didownload sekali jadi
static dalam 400m dari tiap halte TJ (bukan 800m kayak MRT/KRL/LRT - lihat
static_transit.MODE_RADIUS_M, TJ halte-nya jauh lebih rapat), biar backend gak perlu
...
Jalankan: python poi_residential_tj.py
Output: poi_residential_tj_400m.geojson (folder yang sama dengan script ini)
...
halte) - job ini lebih berat dari poi_green_*.py/poi_parking_*.py, DAN TJ punya ~2876
halte (3 kategori GTFS: Angkutan Umum Integrasi, BRT, Transjabodetabek - Mikrotrans dkk
...
poi_residential_tj_work.json - kalau ke-stop di tengah jalan (proses dimatikan paksa dkk),
jalankan lagi `python poi_residential_tj.py`, otomatis lanjut dari stasiun yang belum
selesai. Halte yang gagal (timeout/error Overpass, bukan dimatikan paksa) di-retry
...
permanen mulai run berikutnya - lihat poi_residential_tj_giveup.json - biar halte yang
...
WORK_FILE = OUT_DIR / "poi_residential_tj_work.json"
OUTPUT_FILE = OUT_DIR / "poi_residential_tj_400m.geojson"
FAILED_FILE = OUT_DIR / "poi_residential_tj_400m_failed.txt"
GIVEUP_FILE = OUT_DIR / "poi_residential_tj_giveup.json"
...
# caps it so a halte that fails for a non-transient reason can't hang the script forever;
